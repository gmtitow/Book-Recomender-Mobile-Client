...
public class RegistrationConfirmCodeFragment extends MvpAppCompatFragment implements RegistrationConfirmCodeView {
    
    public static final String REGISTER_LOGIN = "register_login";
    
    @InjectPresenter(type = PresenterType.LOCAL)
    RegistrationConfirmCodePresenter mPresenter;
    
    FragmentRegistrationConfirmCodeBinding mBinding;
    
    public RegistrationConfirmCodeFragment() {
    }
    ...
    private void setupViews(View view) {
        mBinding.editConfirmCode.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                mPresenter.onConfirmCodeChanged(s.toString());
            }
        ...
        mBinding.textSendConfirmToMailAgain.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                mPresenter.replyConfirmCode();
            }
        });
    }
    
    public static Fragment getInstance(String login) {
        RegistrationConfirmCodeFragment fragment = new RegistrationConfirmCodeFragment();
    ...
    @Override
    public void changeSendConfirmButtonEnabled(boolean enabled) {
    ...
    public void showProgress() {
        ...
        changeSendConfirmButtonEnabled(false);
        mBinding.textSendConfirmToMailAgain.setEnabled(false);
        mBinding.textChangeLogin.setEnabled(false);
    }
    
    @Override
    public void hideProgress() {
        mBinding.progress.hide();
        changeSendConfirmButtonEnabled(true);
        mBinding.textSendConfirmToMailAgain.setEnabled(true);
        mBinding.textChangeLogin.setEnabled(true);
    }
        ...
        if(getActivity()!= null) {
            if(seconds > 60){
                ...
                mBinding.textReplyTime.setText(getActivity().getResources().getString(
                        R.string.authentication_registration_confirm_code_enter_code_reply_2, mins, secs));
            } else
                mBinding.textReplyTime.setText(getActivity().getResources().getString(
